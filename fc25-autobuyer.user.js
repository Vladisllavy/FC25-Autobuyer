...
(function () {
    ...
    window.searchFutMarket = function (sender, event, data) {
        ...
        var searchCriteria = window.prepareSearchCriteria(window.autobuyerController.viewmodel.searchCriteria)

        services.Item.searchTransferMarket(searchCriteria, 1).observe(this, (function (sender, response) {
            if (response.success) {
                writeToDebugLog('Найдено ' + response.data.items.length + ' предметов.');
                ...
                writeToLog('Ошибка: Не удалось завершить поиск! Пожалуйста перезагрузите страницу и запустите заново!');
                window.autoBuyerStatus = window.AB_STATUSES.IDLE;
    ...
    window.adjustPrice = function (items) {
        var searchCriteria = window.autobuyerController.viewmodel.searchCriteria;
    ...
    window.adjustProcess = function (price) {
        ...
        if (window.adjust[price] === undefined) {
            writeToLog("Попробуйте установить значение цены на " + price);
        ...
        if (!window.adjustSellPrice) {
            ...
            writeToLog('Цена продажи: ' + window.adjustSellPrice);
        }
        ...
        if (!window.adjustSearchPrice) {
            ...
            writeToLog('Цена поиска: ' + window.adjustSearchPrice);
        }
    ...
    window.updateTransferList = function () {
        services.Item.requestTransferItems().observe(this, function (t, response) {
            ...
            if (minSoldCount > 0 && window.futStatistics.soldItems >= minSoldCount) {
                writeToLog(window.futStatistics.soldItems + " предметов продано");
    ...
    window.activateAutoBuyer = function () {
        ...
        window.setAdjustMode();
        window.autoBuyerStatus = window.AB_STATUSES.ACTIVE;
        window.notify('Автопокупка запущена');
    ...
    window.deactivateAutoBuyer = function () {
        ...
        window.autoBuyerStatus = window.AB_STATUSES.IDLE;
        window.notify('Автопокупка остановлена');
    ...
    window.UTAutoBuyerViewController.prototype.init = function () {
        ...
        const stopButton = window.createButton("Стоп", function () {
        ...
        view.__root.style = "width: 60%; height: 95%;; float: left;";
    ...
    UTAutoBuyerViewController.prototype.getNavigationTitle = function () {
        setTimeout(() => {
            $("#autobuyer-container").parent().prepend(
                ...
                '   <h1 class="title">Статус: <span id="ab_status"></span> | Поисков выполнено: <span id="ab_request_count">0</span> | Профит: <span id="profit_count">0</span></h1>' +
                ...
                '               <div style="float: left;">Поиск:</div>' +
                ...
                '           </div>' +
                '           <div class="view-navbar-clubinfo-name">' +
                '               <div style="float: left;">Статистика:</div>' +
                '               <div style="float: right; height: 10px; width: 100px; background: #888; margin: 5px 0px 5px 5px;">' +
                ...
                '   <div class="view-navbar-clubinfo">' +
                '       <div class="view-navbar-clubinfo-data">' +
                '           <span class="view-navbar-clubinfo-name">Проданные: <span id="ab-sold-items"></span></span>' +
                '           <span class="view-navbar-clubinfo-name">Непроданные: <span id="ab-unsold-items"></span></span>' +
                '       </div>' +
                '   </div>' +
                '   <div class="view-navbar-clubinfo" style="border: none;">' +
                '       <div class="view-navbar-clubinfo-data">' +
                '           <span class="view-navbar-clubinfo-name">Доступно предметов: <span id="ab-available-items"></span></span>' +
                '           <span class="view-navbar-clubinfo-name">Активные трансферы: <span id="ab-active-transfers"></span></span>' +
                '       </div>' +
                '   </div>' +
                '</div>'
            );
            ...
            $("#autobuyer-container").parent().append('' +
                ...
                '    <label>Результаты поиска:</label><br/>' +
            ...
            $('#autobuyer-container .ut-pinned-list').append().append(
                ...
                '   <h1 class="secondary">Настройки:</h1>' +
                '</div>' +
                ...
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <textarea type="tel" class="ut-number-input-control" id="ab_buy_allow_filter" placeholder="messi" style="background-color: #0f0f0f; color: #fcfcf7; width: 100%; height: 70px;"></textarea>' +
                '       </div>' +
                '   </div>' +
                '</div>' +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Не покупать:</span>' +
                '   </div>' +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <textarea class="ut-number-input-control" id="ab_buy_deny_filter" placeholder="messi" style="background-color: #0f0f0f; color: #fcfcf7; width: 100%; height: 70px;"></textarea>' +
                '       </div>' +
                '   </div>' +
                '</div>' +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Продавать:</span>' +
                '   </div>' +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <textarea type="tel" class="ut-number-input-control" id="ab_sell_allow_filter" placeholder="messi" style="background-color: #0f0f0f; color: #fcfcf7; width: 100%; height: 70px;"></textarea>' +
                '       </div>' +
                '   </div>' +
                '</div>' +
                '<div class="price-filter">' +
                '   <div class="info">' +
                '       <span class="secondary label">Не продавать:</span>' +
                '   </div>' +
                '   <div class="buttonInfo">' +
                '       <div class="inputBox">' +
                '           <textarea class="ut-number-input-control" id="ab_sell_deny_filter" placeholder="messi" style="background-color: #0f0f0f; color: #fcfcf7; width: 100%; height: 70px;"></textarea>' +
                '       </div>' +
                '   </div>' +
                '</div>' +
                '<div class="search-price-header">' +
    ...
    function addTabItem() {
        ...
        UTGameTabBarController.prototype.initWithViewControllers = function (tabs) {
            ...
            autoBuyerTab.setText('Автопокупка');
